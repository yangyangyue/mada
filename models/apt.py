...
class AptNet(nn.Module):
    def __init__(self, channels=256, n_layers=4, window_size=1024, patch_size=8, patch_stride=4) -> None:
        ...
        self.n_patch = window_size // patch_stride
        pad_len = window_size - patch_size - (self.n_patch - 1) * patch_stride
    ...
    def forward(self, e, x, gt_apps=None):
        """
        Args:
            examples (N, 3, L): input examples
            samples (N, L): input samples
        """
        e = e.unfold(dimension=-1, size=self.patch_size, step=self.patch_stride).permute(0, 2, 1)
