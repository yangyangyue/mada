...
class AcvaeNet(nn.Module):
    def __init__(self, mid_channels=64, out_channels=256, window_size=1024):
        ...
        self.layer14_1 = nn.Conv1d(out_channels, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, examples, aggs, apps=None):
        ...
        mid, _ = self.atten8(x80.permute(0, 2, 1), x70.permute(0, 2, 1), x70.permute(0, 2, 1))
        x81 = self.layer8_1(mid.permute(0, 2, 1))
        x90 = self.layer9_0(x81)
        mid, _ = self.atten9(x90.permute(0, 2, 1), x60.permute(0, 2, 1), x60.permute(0, 2, 1))
        x91 = self.layer9_1(mid.permute(0, 2, 1))
        x100 = self.layer10_0(x91)
        mid, _ = self.atten10(x100.permute(0, 2, 1), x50.permute(0, 2, 1), x50.permute(0, 2, 1))
        x101 = self.layer10_1(mid.permute(0, 2, 1))
        x110 = self.layer11_0(x101)
        mid, _ = self.atten11(x110.permute(0, 2, 1), x40.permute(0, 2, 1), x40.permute(0, 2, 1))
        x111 = self.layer11_1(mid.permute(0, 2, 1))
        x120 = self.layer12_0(x111)
        mid, _ = self.atten12(x120.permute(0, 2, 1), x30.permute(0, 2, 1), x30.permute(0, 2, 1))
        x121 = self.layer12_1(mid.permute(0, 2, 1))
        x130 = self.layer13_0(x121)
        mid, _ = self.atten13(x130.permute(0, 2, 1), x20.permute(0, 2, 1), x20.permute(0, 2, 1))
        x131 = self.layer13_1(mid.permute(0, 2, 1))
        x140 = self.layer14_0(x131)
        mid, _ = self.atten14(x140.permute(0, 2, 1), x10.permute(0, 2, 1), x10.permute(0, 2, 1))
        x_pre = torch.relu(self.layer14_1(mid.permute(0, 2, 1))).squeeze(1)
        if self.training:
