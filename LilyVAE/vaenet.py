...
class VaeNet(nn.Module):
    ...
    def __init__(self, main_mean, main_std, app_mean, app_std):
        super(VaeNet, self).__init__()
        self.main_mean, self.main_std, self.app_mean, self.app_std = main_mean, main_std, app_mean, app_std

        # encoder
        self.layer1_0 = VaeEncoderBlock(in_channel=1, kernel_size=3, stride=1, res=False)
        ...
        # decoder
        self.layer8_0 = VaeEncoderBlock(in_channel=1, kernel_size=3, stride=1, res=False, in_=False)
    ...
    def forward(self, aggs, apps=None, _=None):
        """ vae forward, she shape of aggs is `(bs, 1, L)` """
        x = (aggs - self.main_mean) / self.main_std
        # encoder
        x10 = self.layer1_0(x)
        ...
        x_pre = self.relu_layer(self.layer14_1(mid_cat))
        if self.training:
            y = (apps - self.app_mean) / self.app_std
            return self.__loss(y, x_pre, mu, logvar)
        else:
            return (x_pre * self.app_std) + self.app_mean
